...
export function ShippingInstructionPrimaryButtons() {
  const { viewMode, setViewMode, setSelectedSiId } = useShippingInstructions()
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      toast.success('Shipping instructions list refreshed!')
    }, 800)
  }
  ...
  return (
    <div className='flex items-center gap-2'>
      ...
      <Button
        ...
        onClick={() => setSelectedSiId('new')}
      >
        <Plus className='size-3.5' />
        <span className='text-xs'>Create Shipping Instruction</span>
      </Button>
    </div>
  )
}
