...
export function PurchaseOrdersPrimaryButtons() {
  const { viewMode, setViewMode, setSelectedPoId } = usePurchaseOrders()
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      toast.success('Purchase Orders list refreshed!')
    }, 800)
  }
      ...
      <Button
        ...
        onClick={() => {
          if (purchaseOrders.length > 0) {
            ...
            toast.info('Opened Purchase Order in editor mode.')
          }
        }}
      >
        <Plus className='size-3.5' />
        <span className='text-xs'>Create Purchase Order</span>
      </Button>
    </div>
  )
}
