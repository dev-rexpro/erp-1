...
export function VendorRatesPrimaryButtons() {
  const { viewMode, setViewMode } = useVendorRates()
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      toast.success('Vendor rates list refreshed!')
    }, 800)
  }
    ...
    <div className='flex items-center gap-2'>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant='outline' size='sm' className='h-8 px-3 gap-1.5'>
            ...
            <span className='text-xs font-medium capitalize'>
              {viewMode === 'table' ? 'List View' : viewMode === 'report' ? 'Report View' : 'Card Grid View'}
            </span>
        ...
        <DropdownMenuContent align='start' className='w-[180px] rounded-xl'>
          ...
          <DropdownMenuItem onClick={() => setViewMode('cards')} className='flex items-center gap-2 cursor-pointer'>
            <Kanban className='size-4 text-muted-foreground' /> Card Grid View
            {viewMode === 'cards' && <Check className='size-3.5 ml-auto' />}
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      <Button
        ...
        onClick={() => toast.success('New carrier tariff contract wizard opened.')}
      >
        <Plus className='size-3.5' />
        <span className='text-xs'>Add Rate Card</span>
      </Button>
    </div>
  )
}
