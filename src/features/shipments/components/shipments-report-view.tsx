...
interface ShipmentsReportViewProps {
  data: Shipment[]
...
const COLUMNS_CONFIG = [
  ...
  { id: 'username', label: 'Shipment No', minWidth: 130, filterable: true, sortable: true },
  { id: 'name', label: 'Client Account', minWidth: 160, filterable: true, sortable: true },
  { id: 'role', label: 'Carrier / Mode', minWidth: 150, filterable: true, sortable: false },
  { id: 'amount', label: 'Freight Value', minWidth: 120, filterable: true, sortable: true },
  { id: 'validUntil', label: 'Delivery ETA', minWidth: 120, filterable: true, sortable: true },
  { id: 'status', label: 'Status', minWidth: 120, filterable: true, sortable: false },
] as const

export function ShipmentsReportView({ data }: ShipmentsReportViewProps) {
    ...
    if (sortCol) {
      result.sort((a, b) => {
        ...
        if (sortCol === 'name') {
          ...
          valA = (a[sortCol as keyof Shipment] as string || '').toLowerCase()
          valB = (b[sortCol as keyof Shipment] as string || '').toLowerCase()
        }
  ...
  const groupedData = useMemo(() => {
    ...
    processedData.forEach((row) => {
      ...
      if (groupBy === 'name') {
        ...
        const carrier = roles.find(r => r.value === row.role)
        key = carrier?.label || row.role
        ...
        key = (row[groupBy as keyof Shipment] as string) || 'No Group Specified'
      }
      ...
      <div className='relative flex flex-col flex-1 border rounded-md bg-background overflow-hidden'>
          ...
          <div className='absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-4 py-2 bg-popover border text-popover-foreground rounded-full shadow-lg border-primary/20 animate-in fade-in slide-in-from-bottom-2 duration-150'>
            ...
            <div className='flex items-center gap-1'>
              <Button
                ...
                onClick={() => toast.success(`Shipments manifest exported for ${selectedCount} items.`)}
              >
                Export Manifest
              </Button>
              <Button
                      ...
                      <TableHead key='filter-status' className='px-1 py-1.5 border-r align-middle'>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value='active' className='text-xs'>In Transit</SelectItem>
                            <SelectItem value='inactive' className='text-xs'>Delivered</SelectItem>
                            <SelectItem value='invited' className='text-xs'>Scheduled</SelectItem>
                            <SelectItem value='suspended' className='text-xs'>Delayed</SelectItem>
                          </SelectContent>
                        </Select>
                      </TableHead>
                    )
        ...
        <div className='flex items-center justify-between px-4 py-2 border-t bg-muted/20 font-sans text-[11px] text-muted-foreground/75'>
          <div className='flex items-center gap-2.5'>
            <span>CAPACITY: 500 shipments</span>
          ...
          <div className='flex items-center gap-1.5'>
            <Badge variant='outline' className='bg-background text-[10px] text-muted-foreground border-none tracking-tight gap-1 px-1.5 py-0'>
              ...
              <span>Freight Ledger Synced</span>
            </Badge>
          </div>
        </div>
...
const SpreadsheetRow = React.memo(function SpreadsheetRow({
  ...
  return (
    <TableRow className={cn('h-10 group', isSelected ? 'bg-muted/20' : '')}>
      ...
      >
        <span className='capitalize truncate'>{carrier?.label || row.role}</span>
      </TableCell>
