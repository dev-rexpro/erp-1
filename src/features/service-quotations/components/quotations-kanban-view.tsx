...
export function QuotationsKanbanView({ data, search }: QuotationsKanbanViewProps) {
  ...
  const setQuotations = useDbStore((state) => state.setQuotations)
  ...
  const handleMoveCard = (id: string, newStatus: QuotationStatus) => {
    const itemIndex = quotations.findIndex((u) => u.id === id)
    if (itemIndex > -1) {
      const updatedQuotations = [...quotations]
      updatedQuotations[itemIndex] = { ...updatedQuotations[itemIndex], status: newStatus }
      setQuotations(updatedQuotations)
      setViewMode((prev) => prev)
      toast.success(`Moved ${updatedQuotations[itemIndex].username} to ${newStatus}`)
    }
  }
