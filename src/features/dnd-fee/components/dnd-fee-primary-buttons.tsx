...
export function DndFeePrimaryButtons() {
  const { viewMode, setViewMode, setSelectedDndId } = useDndFee()
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      toast.success('D&D Fee list refreshed!')
    }, 800)
  }
  ...
  return (
    <div className='flex items-center gap-2'>
      ...
      <Button
        ...
        onClick={() => setSelectedDndId('new')}
      >
        <Plus className='size-3.5' />
        <span className='text-xs'>Record D&D Fee</span>
      </Button>
    </div>
  )
}
