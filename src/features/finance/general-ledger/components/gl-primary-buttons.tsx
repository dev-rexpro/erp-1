...
export function GLPrimaryButtons() {
  const { viewMode, setViewMode, setSelectedVoucherId } = useGL()
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      toast.success('General ledger list refreshed!')
    }, 1000)
  }
  ...
  return (
    <div className='flex items-center gap-2'>
      ...
      <Button
        onClick={() => setSelectedVoucherId('new')}
        size='sm'
      ...
      >
        <Plus size={15} />
        <span className='text-xs'>New Voucher</span>
      </Button>
    </div>
  )
}
